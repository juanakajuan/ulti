...
/// Mutable RGBA framebuffer plus its dimensions.
...
impl<'a> Framebuffer<'a> {
    /// Creates a framebuffer view over the raw RGBA pixel slice.
    ...
    fn draw_text(&mut self, font: &Font, text: &str, x: u32, y: u32) {
        let mut pen_x = x;
        for character in text.chars() {
            ...
            for glyph_row in 0..metrics.height {
                for glyph_col in 0..metrics.width {
                    ...
                    if pixel_x < 0
                        || pixel_y < 0
                        || pixel_x >= i64::from(self.width)
                        || pixel_y >= i64::from(self.height)
                    {
                        continue;
                    }

                    let offset = ((pixel_y as u32 * self.width + pixel_x as u32) * 4) as usize;
                    blend_pixel(&mut self.pixels[offset..offset + 4], FOREGROUND, alpha);
                }
            }
    ...
    fn fill_rect(&mut self, x: u32, y: u32, rect_width: u32, rect_height: u32, color: [u8; 4]) {
        let max_y = (y + rect_height).min(self.height);
        let max_x = (x + rect_width).min(self.width);
        for pixel_y in y..max_y {
            for pixel_x in x..max_x {
                let offset = ((pixel_y * self.width + pixel_x) * 4) as usize;
    ...
    for row in 0..rows {
        for col in 0..cols {
            ...
            if row == cursor.0 && col == cursor.1 {
                framebuffer.fill_rect(x, y, CELL_WIDTH, CELL_HEIGHT, CURSOR);
            }
            ...
            framebuffer.draw_text(font, &contents, x, y);
        }
    }
}

/// Converts a terminal cell coordinate to a framebuffer pixel origin.
fn cell_origin(row: u16, col: u16) -> (u32, u32) {
    (
...
fn blend_pixel(pixel: &mut [u8], color: [u8; 4], alpha: u8) {
    ...
    for channel in 0..3 {
        pixel[channel] = ((u16::from(color[channel]) * alpha
            + u16::from(pixel[channel]) * inverse_alpha)
            / 255) as u8;
    }
    pixel[3] = 255;
}
